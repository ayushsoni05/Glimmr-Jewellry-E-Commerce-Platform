...
/*──────────────────────────────────────────────────────────────────────
 *  CINEMATIC  WHITE-GLOVE  DOORSTEP  DELIVERY  VIDEO  MODAL
 *  —  Seamless 60fps MP4 Film with Live Synchronized Narration  —
 *──────────────────────────────────────────────────────────────────────*/

const VIDEO_SRC = '/assets/animation/delivery_story.mp4';
...
const JewelryOrderStoryModal = ({ isOpen, orderData, onClose }) => {
  ...
  // Synchronize subtitles with video playback time
  const handleTimeUpdate = () => {
    if (!videoRef.current) return;
    const time = videoRef.current.currentTime;
    if (time < 2.4) setCurrentSceneIdx(0);
    else if (time < 4.8) setCurrentSceneIdx(1);
    else if (time < 7.2) setCurrentSceneIdx(2);
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="relative w-full max-w-2xl my-auto flex flex-col items-center z-10">
          ...
          {/* ═══════ VIDEO STORY STAGE (Active while playing) ═══════ */}
          {!videoEnded && (
            <motion.div
