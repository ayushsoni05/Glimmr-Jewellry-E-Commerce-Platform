...
async function fetchPerGramRates(currency = 'INR') {
  ...
  try {
    ...
    console.warn('[ORDER RATES] GoldAPI.io failed, trying MetalPriceAPI:', err.message);
  }
  ...
  if (!goldPerGram || goldPerGram <= 0) {
    try {
      ...
      console.warn('[ORDER RATES] MetalPriceAPI also failed, using fallback:', err.message);
    }
  }
